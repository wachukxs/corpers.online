...
app.get('/user/:sc', function (req, res) {
...
app.post('/login', function(req, res /*, handleRedirect*/){
  ...
  connection.query(sqlquery, function (error, results, fields) {
    ...
    if ( isEmpty(results) ) {
      ...
      req.session.user = req.body ;
      z = req.body.email;
      res.redirect('/user/' + results[0].email);
    }
  });
...
io.of('/user').on('connection', function (socket) {// when a new user is in the login page
  ...
  socket.on('cameOnline', function (data) {
    ...
    //io.emit sends to EVERYONE
    socket.broadcast.emit('boardcast message', { will: 'be received by everyoneELSE', msg: 'new login', from: 'user ' + data.nowOnline + ' on ' + socket.client.id + ' just came online, say hi!!!!!!! and share info.'});
  });

  //when we receive 'boardcast message' from any connected socket
  socket.on('boardcast message', function(data){
    console.log(data);
    socket.broadcast.emit('boardcast message', { what: 'a post', of: data.text});
  });
